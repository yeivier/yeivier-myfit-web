import type { MesocycleExerciseTemplateWithoutIdsOrIndex } from '@/lib/types/exerciseTemplates';
import type { ActiveMesocycleWithProgressionData, WorkoutExerciseWithSets as WorkoutExerciseWithSetsPayload } from '@/lib/server/includes';
...
export function getExerciseVolume(workoutExercise: WorkoutExerciseWithSetsPayload, userBodyweight: number) {
	return arraySum(
...
/** Fórmula de Berger: relaciona carga, repeticiones y RIR para estimar el % de sobrecarga entre dos series. */
...
export function getWorkoutVolume(workout: { userBodyweight: number; workoutExercises: WorkoutExerciseWithSetsPayload[] }) {
...
type PreviousPerformance = {
	exercise: WorkoutExerciseWithSetsPayload;
	oldUserBodyweight: number;
};
...
export type WorkoutExerciseWithSets = WorkoutExerciseWithSetsPayload;
...
function generateAveragePerformanceDropOffs(performances: PreviousPerformance[]) {
	...
	// Estimaciones incorrectas de RIR causando un aumento en el rendimiento con el tiempo
	if (invalidDropOffs) {
...
function addExtraSetProperties(exerciseSet: WorkoutExerciseWithSetsPayload['sets'][number]) {
...
function getPerformanceChanges(performances: { exercise: WorkoutExerciseWithSetsPayload; oldUserBodyweight: number }[]) {
...
function increaseLoadOfSets(ex: WorkoutExerciseInProgress, userBodyweight: number) {
	...
	const newSets = ex.sets.map((set, setIdx) => {
		...
		// Para TopBackoff, usar topRepRangeEnd en la primera serie, repRangeEnd normal en las demás
...
/**
 * El corazón de MyFit: calcula automáticamente la progresión (reps, carga, series) del próximo
 * entrenamiento a partir del historial del mesociclo activo.
 */
export function progressiveOverloadMagic(
	...
	// Quitar IDs de mini-series y desmarcar todas las series como no saltadas
	workoutExercises.forEach((ex) => {
		ex.sets.forEach((set) => {
